...
public class SlackHomeService
{
    ...
    {
        var query = _context.ActionItems.Where(a => a.UserId == userId && a.Type != ActionItemType.Deadline);
        ...
        var blocks = BuildHomeBlocks(items, cCount, rCount, qCount, filter, sort);

        await SendToSlack(userId, blocks);
    }

    private List<object> BuildHomeBlocks(List<ActionItem> items, int c, int r, int q, string? filter, string? sort)
    {
        var blocks = new List<object> {
            new { type = "header", text = new { type = "plain_text", text = ":clipboard:Your Action Dashboard" } },
            new {
                type = "section",
                text = new { type = "mrkdwn", text = $"*Stats:* {c} Commitments | {r} Requests | {q} Questions" }
            },
            new { type = "divider" },
            new {
                type = "actions",
        ...
        {
            string formattedText = UI.SlackMessageBuilder.FormatItemText(item);
            ...
            blocks.Add(new
            {
                ...
                {
                    type = "mrkdwn",
                    text = $"{UI.SlackMessageBuilder.GetEmoji(item.Type)} {formattedText}\n_Added {item.CreatedAt:dd.MM.yyyy}_ _Type: {item.Type}_"
                }
            });
